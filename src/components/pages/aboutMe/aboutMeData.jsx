export const WORK_EXPERIENCE = [
  {
    id: 1,
    role: "Software Engineer",
    company: "Sprobe Inc.",
    period: "April 2022 – March 2026",
    type: "full-time",
    description:
      "Oversaw batch job processes, tracked execution status, and reported errors to ensure operational reliability. Logged into Linux servers to extract data, processed and organized information, and prepared structured reports in Excel for analysis. Uploaded and maintained files on developer websites, supporting project workflows and ensuring timely resource availability. Executed PostgreSQL queries to confirm successful file uploads, maintaining data integrity and accuracy across systems. Assigned to a template project to gain hands‑on experience with React and Laravel technologies, focusing on building and integrating modern web applications.",
    tags: ["Linux", "postgreSQL", "HTML", "CSS", "JavaScript", "Git", "React", "Material UI", "Laravel"],
  },
];

export const EDUCATION = [

  {
    id: 1,
    degree: "Bachelor of Science in Computer Engineering",
    school: "University of Cebu Main Campus",
    period: "2016 – 2021",
    description:
      "Graduated with a focus on web development and software engineering fundamentals.",
  },
];

export const CERTIFICATES = [
  {
    id: 7,
    title: "React, NodeJS, Express MongoDB - The MERN Fullstack Guide",
    issuer: "Udemy",
    date: "May 20, 2026",
    icon: "⚛️",
    color: "bg-[#E8EDFB]",
  },
  {
    id: 6,
    title: "React — The Complete Guide (incl. Next.js, Redux)",
    issuer: "Udemy",
    date: "July 09, 2026",
    icon: "⚛️",
    color: "bg-[#E8EDFB]",
  },

  {
    id: 5,
    title: "AWS Cloud Practitioner Essentials",
    issuer: "AWS",
    date: "June 11, 2025",
    icon: "💻",
    color: "bg-[#EDE9FE]",
  },
  {
    id: 4,
    title: "AWS Cloud Quest: Cloud Practitioner",
    issuer: "AWS",  
    date: "July 03, 2025",
    icon: "🗄️",
    color: "bg-[#E8F5E9]",               
  },
  {
    id: 3,
    title: "Getting Started with AWS Cloud Essentials",
    issuer: "AWS",
    date: "July 09, 2025",
    icon: "🏗️",
    color: "bg-[#F0F9FF]",
  },
  {
    id: 2,
    title: "AWS Technical Essentials",
    issuer: "AWS",
    date: "February 03, 2026",
    icon: "🔧",
    color: "bg-[#FFF3E0]",
  },
  {
    id: 1,
    title: "AWS Escape Room: Exam Prep for AWS Certified Cloud Practitioner (CLF-C02) (Demo) ",
    issuer: "AWS",
    date: "February 04, 2026",
    icon: "🏢",
    color: "bg-[#E8F5E9]",
  }

];
